...
func NewSpecies(bitsPerGene, numGenes uint8, signedGenes ...uint8) (*Species, error) {
...
func (s *Species) SerializeChromosome(gene Chromosome) (uint64, error) {
	...
	mask := s.chromosomeMask()
	var serialized uint64
	for _, allele := range gene.Genes {
		serialized = (serialized << s.BitsPerGene) + (uint64(allele) & mask)
	}

	return serialized, nil
}

// chromosomeMask calculates the dynamic mask to use based on BitsPerGene
func (s *Species) chromosomeMask() uint64 {
	unusedBits := uint64(64 - s.BitsPerGene)
	mask := ^uint64(0) // QWORD of 1s
	return mask >> unusedBits
}
